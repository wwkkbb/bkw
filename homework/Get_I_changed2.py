    ...
    if I_all_index in chongtu:
        for project in I_all[I_all_index]:
            if project[2]!=project[3] and project[1][project[2]] in project[4]:#移进
                Go_dic[(I_all_index,project[1][project[2]])]=[Go_dic[(I_all_index,project[1][project[2]])],0]#0表示移进
            else:
                if [project[0], project[1]] ==[I_start[0][0],I_start[0][1]]:
                    Go_dic[(I_all_index,TERMINATOR)]=[[project[0],project[1]],2]#1表示规约接受
                else:
