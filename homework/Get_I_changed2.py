...
# grammars = {
#     'z':['Z'],
#     'Z': ['S', 'P'], 'S': ['i(B)S', 'w(B)S;', 'B;', '{S}', 'SS'], 'B': ['I=E', 'E'],
#     'E': ['E+T', 'E-T', 'T'], 'T': ['T*F', 'T/F', 'F'], 'F': ['I', 'n', '(E)'], 'P': ['QK;'],
#     'K': ['I=n', 'I', 'I,K', 'I=n,K'], 'Q': ['c', 'f', 'k']
# }
grammars = {
    'z':['Z'],
    #语句分为执行语句和声明语句
    ...
    if I_all_index in chongtu:
        for project in I_all[I_all_index]:
            if project[2] != project[3] and project[1][project[2]] in project[4]:  # 移进
                Go_dic[(I_all_index, project[1][project[2]])] = [Go_dic[(I_all_index, project[1][project[2]])],
                                                                 0]  # 0表示移进
            else:
        ...
        for project in I_all[I_all_index]:
            if project[2] != project[3]:  # 移进
                ...
                if [project[0:3]] == [I_start[0][0], I_start[0][1],project[3]]:
...
for I_all_index in range(len(I_all)):
    for every_vt in vt_vn_TERMINOR:
        if (I_all_index, every_vt) in Go_dic:
            print(Go_dic[(I_all_index, every_vt)], sep=' ')
        else:
