...
def save_token(ls,path_token):
    ...
    return token_list
def save_namel(dict_namel,path_namel):
    namel_list=[]
    for x in list(dict_namel):
        namel_list.append([x,dict_namel[x]])
    print("namel表：")
    for x in namel_list:
        print(x)
    fileObject = open(path_namel, 'w')
    for ip in namel_list:
        fileObject.write(str(ip))
        fileObject.write('\n')
    return namel_list
...
if x=='1':
    print(r"请输入文件名：如：'E:\studyclass3first\Compilers_Principles\code\dist\token.txt'")
    path = input()
    files= open(path)
    st=files.read()
    pass
...
print("token表含义：单词，编码类，语义（若为consl,则表示索引，数值。若为namel,则为标识符，数值） ")
